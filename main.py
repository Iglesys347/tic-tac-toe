...
if choice == "1":
    ...
    while not game_finished:
        print(f"Player {game.get_current_player} please enter the cell number in which you want to play (you can show the diffent cell numbers by typing '?'")
        comm = input()
        if comm == "?":
    ...
    while not game_finished:
        if  game.current_player == player_drawing:
            ...
            print(f"{ia.get_name} : I'll play here")
            game_finished = game.turn(ia.dummy_play())
            game.switch_player()
print(f"Well done {get_other_drawing(game.current_player)} you won !")